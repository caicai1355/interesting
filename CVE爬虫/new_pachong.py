...
def cve_analyse(content):
	...
	if cnnveMsg == None:
		...
		strTemp = "   " + cnnveMsg.group(4).replace('&nbsp;','').replace('<br/>','').replace('<p>','\n').replace('</p>','').replace('\'','\\\'').replace('\"','\\\"')
	file.write(strTemp + '\n')
	print strTemp.decode('UTF-8').encode('GB18030')
	...
	# 	strTemp = "   暂无公告与补丁"
	# else:
	# 	reportAndHotfixArray = pattern21.split(reportAndHotfix.group(1))
	# 	reportAndHotfixStr = "".join(reportAndHotfixArray).replace('&nbsp;','').replace('<br/>','\n').replace('</a>','\n').replace('<ul>','').replace('<li>','').replace('</ul>','').replace('</li>','')
...
def url_filter(url):
	...
	try:
		...
		if cveId:
			if ret == () or cveId.group(1) > ret[0][0]:	
				return True
			else:
				return False
		else:
...
def cve_filter(url):
	...
	try:
		cveId = patternUrl.search(url)
		ret = cur.execute("select cveid from cve where cveid = '" + cveId.group(1) + "'")
